...
import { createWorkflowSleepSignal } from "./control.js";
...
import { parseTimerDuration, validateId } from "./validation.js";
...
export function createStepTools(
  ...
  return {
    async run<T>(stepId: string, fn: () => Promise<T> | T): Promise<T> {
      begin(stepId);
      try {
        if (await persist(() => adapter.getTimer(run.id, stepId))) {
          throw new ValidationError(`step '${stepId}' is already used by a sleep`);
        }
        const rawExisting = adapter.getStepRaw
          ? await persist(() => adapter.getStepRaw!(run.id, stepId))
          : undefined;
        const existing = rawExisting ?? (await persist(() => adapter.getStep(run.id, stepId)));
    ...
    async sleep(stepId, duration): Promise<void> {
      ...
      try {
        await sleepUntil(
          stepId,
          new Date(Date.now() + parseTimerDuration(duration, "sleep duration"))
        );
      } finally {
        finish(stepId);
      }
  ...
  async function sleepUntil(stepId: string, fireAt: Date): Promise<void> {
    ...
    if (await persist(() => adapter.getStep(run.id, stepId))) {
      throw new ValidationError(`step '${stepId}' is already used by step.run`);
    }
    const timer = await persist(() =>
      adapter.sleepRun({
        ...ownership,
        stepId,
        fireAt,
    ...
    if (timer.status === "pending")
      throw createWorkflowSleepSignal(`workflow sleeping until ${timer.fireAt.toISOString()}`);
  }
}
