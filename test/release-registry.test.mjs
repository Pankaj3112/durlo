...
test("reads an existing scoped exact version as JSON without the abbreviated metadata header", async () => {
  let request;
  const result = await readRegistryPackage("@durlo/core", "0.1.0-alpha.0", async (url, options) => {
    request = { url, options };
    return {
      ok: true,
      status: 200,
      json: async () => ({ name: "@durlo/core", version: "0.1.0-alpha.0" })
    };
  });

  assert.equal(
    request.url,
    "https://registry.npmjs.org/%40durlo%2Fcore/0.1.0-alpha.0"
  );
  assert.notEqual(
...
test("treats an exact 404 as unpublished", async () => {
  const result = await readRegistryPackage("@durlo/core", "0.1.0-alpha.0", async () => ({
    ok: false,
    status: 404
  }));
